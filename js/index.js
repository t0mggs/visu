...
function addVisuBloqBranding(pdf, isHighQuality) {
    ...
    for (let i = 0; i < pdf.internal.getNumberOfPages(); i++) {
        ...
        // Barra superior con gradiente azul VisuBloq
        pdf.setFillColor(0, 123, 255); // Azul VisuBloq
        pdf.rect(0, 0, pageWidth, isHighQuality ? 35 : 18, 'F');
        
        // Logo y título principal
        pdf.setFontSize(isHighQuality ? 18 : 9);
        pdf.setTextColor(255, 255, 255); // Blanco sobre azul
        pdf.text(15, isHighQuality ? 20 : 10, "🧱 VisuBloq");
        
        // Subtítulo en la barra superior
        pdf.setFontSize(isHighQuality ? 11 : 6);
        pdf.setTextColor(220, 230, 255); // Azul claro
        pdf.text(15, isHighQuality ? 30 : 15, "Instrucciones LEGO Personalizadas");
        
        // Información de página en la barra superior (derecha)
        pdf.setFontSize(isHighQuality ? 10 : 5);
        pdf.setTextColor(255, 255, 255);
        const pageText = `${i + 1} / ${pdf.internal.getNumberOfPages()}`;
        const pageTextWidth = pdf.getTextWidth(pageText);
        pdf.text(pageWidth - pageTextWidth - 15, isHighQuality ? 25 : 12, pageText);
        
        // Línea decorativa bajo la barra
        pdf.setDrawColor(0, 100, 200);
        pdf.setLineWidth(isHighQuality ? 1 : 0.5);
        pdf.line(0, isHighQuality ? 35 : 18, pageWidth, isHighQuality ? 35 : 18);
        
        // Footer minimalista
        const footerY = pageHeight - (isHighQuality ? 15 : 8);
        pdf.setFontSize(isHighQuality ? 8 : 4);
        pdf.setTextColor(150, 150, 150);
        pdf.text(15, footerY, "visubloq.com");
        
        // Línea decorativa superior del footer
        pdf.setDrawColor(200, 200, 200);
        pdf.setLineWidth(isHighQuality ? 0.5 : 0.3);
        pdf.line(15, footerY - (isHighQuality ? 8 : 4), pageWidth - 15, footerY - (isHighQuality ? 8 : 4));
        
        // Información adicional en el footer (derecha)
        const footerInfo = "Diseña tu arte LEGO";
        const footerInfoWidth = pdf.getTextWidth(footerInfo);
        pdf.text(pageWidth - footerInfoWidth - 15, footerY, footerInfo);
        
        // Marca de agua muy sutil (solo en alta calidad)
        if (isHighQuality && i > 0) { // No en la primera página
            pdf.setFontSize(60);
            pdf.setTextColor(250, 250, 250); // Casi imperceptible
            const watermarkText = "VisuBloq";
    ...
    runStep4(async () => {
        ...
        for (var i = 0; i < totalPlates; i++) {
            ...
            if ((i + 1) % (isHighQuality ? 20 : 50) === 0) {
                addVisuBloqBranding(pdf, isHighQuality);
